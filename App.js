...
export default function App() {
  const local_coords = { latitude: 23.2504337 , longitude:  77.5002442 }; // isko manually ek fixed position ka dalana
  ...
  function Calculator(){
    ...
    const time_out = now.toLocaleTimeString();
    if (final > 200) {
      console.log("You are away than 200m range");
      sendAttendanceData(currentDate,time_in, time_out, final_lat, final_lon)
  ...
  async function sendAttendanceData( date, timeIn, timeOut, latitude, longitude, empId=1234567890) {
    // console.log(empId=1234567890, date='2024-09-07', timeIn='09:00', timeOut="00:00", latitude, longitude)
    try {
      const response = await axios.patch(`http://192.168.29.91:3000/postattendance/${empId}`, {
        date: date,
        time_in: timeIn,
        time_out: timeOut,
        lat: latitude, 
